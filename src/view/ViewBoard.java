...
/**
 * Controls the look of the board, where all tiles are placed, and how they are
 * moved.
 */
public class ViewBoard extends JLayeredPane {
	...
	/**
	 * Sets the maximum width this board can be as well as sets the width the tiles
	 * must be to fit the constraints.
	 * 
	 * @param numTilesX     The number of tiles that must fit within the given
	 *                      width.
	 * @param maxBoardWidth The maximum width this board can be.
	 * @return The actual board width. This will always be <= the given
	 *         maxBoardWidth
	 */
	...
	/**
	 * Sets the maximum height this board can be as well as sets the height the
	 * tiles must be to fit the constraints.
	 * 
	 * @param numTilesY      The number of tiles that must fit within the given
	 *                       height.
	 * @param maxBoardHeight The maximum height this board can be.
	 * @return The actual board height. This will always be <= the given
	 *         maxBoardHeight
	 */
	...
	/**
	 * Moves the given list of tile actions; moving, adding, and removing them as
	 * dictated.
	 * 
	...
	public void moveTiles(List<ATileMove> moves) {
		for (ATileMove aMove : moves) {
			...
			if (aMove.isDeleted()) {
				...
				if (aMove.getPrvLoc() == null)
					add(new ViewTile(aMove.getCurVal(), coord2Point(aMove.getCurLoc())), TILELAYER); // A newly added
																										// tile
