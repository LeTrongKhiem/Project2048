...
public class BasicGUI extends JFrame implements ViewInterface {
	...
	/**
	 * Creates a GUI for a game with a given number of horizontal and vertical tiles
	 ...
	 * @throws IllegalArgumentException If numTilesX < 4 or numTilesX > 20 or
	 *                                  numTilesY < 4 or numTilesY > 20.
	...
	private synchronized void createAndShowGUI(int numTilesX, int numTilesY) {
		...
		JPanel scoresPanel = new JPanel(new FlowLayout(FlowLayout.RIGHT, 10, 10)); // a panel to hold both the score and
																					// the best score
		...
		// create a container to hold a basic message to the player and a "start new
		// game" button.
...
//			}
		...
		above.add(newGame);
		// Create a container to group 2 combo boxes to allow user to set the size of
		// the board upon game restart
		...
		numTilesXBox.setSelectedIndex(numTilesXBox.getItemCount() - 1);// sets to last element then moves it to the
																		// correct selection
		...
		numTilesYBox.setSelectedIndex(numTilesYBox.getItemCount() - 1);// sets to last element then moves it to the
																		// correct selection
		...
		board = new ViewBoard();
		// Now determine the maximum size that the actual game board can be based on
		// screen size and set the sizes of the other components
