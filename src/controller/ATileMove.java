...
/**
 * Groups information regarding the past and present position and value for a
 * Tile as well as if the Tile was formed from merging Tiles, or if the Tile is
 * to be deleted. Convention assumed: The point information is to be in (x,y)
 * coordinates where (0,0) refers to the top-left cell, while (1,2) would be one
 * cell down and two cells to the right.
 */
public class ATileMove {
	...
	/**
	 * Updates the current location and value of this Tile to the information
	 * provided and marks the Tile as being merged.
	 * 
	...
	public boolean equals(Object obj) {
		if (this == obj)
			return true;
		if (!(obj instanceof ATileMove))
			return false;
		ATileMove that = (ATileMove) obj;
		if (this.curVal != that.curVal)
			return false;
		if (!this.curLoc.equals(that.curLoc))
			return false;
		if (this.prvVal != that.prvVal)
			return false;
		if (this.prvLoc == null) {
			if (that.prvLoc != null)
				return false;
		} else if (!this.prvLoc.equals(that.prvLoc))
			return false;
		return true;
	}
