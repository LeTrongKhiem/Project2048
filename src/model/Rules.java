...
public class Rules {
	...
	/**
	 * Modifies the given board to have the given dimensions and then starts a new
	 * game on that board, returning the tiles that were added to it.
	 * 
	 ...
	 * game on that board, returns the tiles that were added to it. Note this does
	 * not reset the highScore value of the board as the startGame method does.
	 * 
	 * @param board  The board that the Rules are being used to modify.
	 * @param width  The width of the board to be made.
	 * @param height The height of the board to be made.
	 ...
	 *         affected by the move as well as the tile that was added to the board
	 *         (only added after a successful move).
	 */
			...
			for (int y : yTraversal) {
				...
				if (board.valueAt(current) != 0) {
					...
					if (board.isContained(next) && board.valueAt(current) == board.valueAt(next)
							&& canMerge(board, moves, next)) {// merge current & next position
					...
					} else if (current != farthest) {// no merge, just move current to the farthest position. Do nothing
														// if this Tile cannot be moved.
	...
	private static void deleteMergeInto(Board board, List<ATileMove> moves, Point next) {
		...
		if (!found) {// tile being merged into wasn't moved earlier, so now add it in and mark as
						// deleted
